from Classifiers.custom_layers import HyperedgePoolingLayer, MultiheadSelfAttention
...
class HyperSAGNN(BaseModel):
    ...
    def __init__(
        ...
        self.static_embedding = torch.nn.Linear(
            self.dimensions, self.dimensions)
        self.dynamic_embedding = MultiheadSelfAttention(
            self.dimensions, n_heads=num_heads)
    ...
    def forward(self, hyperedges):
        ...
        """
        preds = torch.zeros(len(hyperedges)).to(utils.get_device())
        for idx, hedge in enumerate(hyperedges):
            x = self.node_embeddings[hedge]
            x_static = torch.dropout(
                torch.tanh(self.static_embedding(x)),
                self.dropout, train=self.training)
            x_dynamic = torch.dropout(
                self.dynamic_embedding(x), self.dropout, train=self.training)
            outputs = torch.sigmoid(
                self.classif(torch.pow(x_static - x_dynamic, 2)))
            preds[idx] = outputs.mean()
        return preds
